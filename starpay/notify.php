            ...
            if ($access === 'POST'){
                ...
                if ($status == 'R000'){
                    ...
                    $cart_id = $cart[1];;
          					...
          					$customer = $cart[3];
          					$total = $orderAmt/100;
                    $starpay = Module::getInstanceByName('starpay');
                    $starpay->validateOrder($cart_id, Configuration::get('PS_OS_PAYMENT'), $total, 'AlipayPlus Pay', NULL, array(), $currency_id, false, $customer);
                }
                echo 'SUCCESS';
            }
            else{
                ...
                if ($status == 'R000'){
                    ...
                    $cart_id = $cart[1];;
          					...
          					$total = $orderAmt/100;
                    $starpay = Module::getInstanceByName('starpay');
                    $starpay->validateOrder($cart_id, Configuration::get('PS_OS_PAYMENT'), $total, 'StarPay', NULL, array(), $currency_id, false, $customer);
                }
                echo 'SUCCESS';
            }
        }
